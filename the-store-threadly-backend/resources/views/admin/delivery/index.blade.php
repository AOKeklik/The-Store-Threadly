                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="datatable table table-bordered">
                                    ...
                                    <tbody data-app-section="table-delivery">
    ...
    <script>
        ...
        async function handlerSubmit (e) {
            try {
                ...
                const submit=await submitFrom({url:"{{ route('admin.delivery.store') }}",formData})
                await fetchTable()
        ...
        async function handlerChange (e, id, key) {
            try {
                e.preventDefault()
                ...
                formData.append("status_key",key)
                formData.append("status_value",$(e.target).prop("checked") ? 1 : 0)
                    
                const submit=await submitFrom({url:"{{ route('admin.delivery.status.update') }}",formData})
        ...
        async function handlerDelete (e, id) {
            try {
                ...
                const submit=await submitFrom({url:"{{ route('admin.delivery.delete') }}",formData})
                await fetchTable()
        ...
        function fetchTable(){
            $.ajax({
                type:"GET",
                url:"{{ route('admin.delivery.section.table.view') }}",
                success:function(res){
                    $("[data-app-section=table-delivery]").html(res)
                    reloadJqueryPlugins ()
                },
                error: function(xhr) {
